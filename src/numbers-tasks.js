...
function roundToPowerOfTen(num, pow) {
  ...
  if (+numArr[pow - 1] >= 5) {
    newArr.push((+numArr[pow] + 1).toString());
    newArr.push(...numArr.slice(pow + 1));
    return +newArr.reverse().join('');
  }
  newArr.push(...numArr.slice(pow));
  return +newArr.reverse().join('');
}
/**
...
function getSumOfDigits(num) {
  if (num.toString().length > 1) {
    ...
    return numArr.reduce((sum, element) => sum + +element, 0);
  }
  return num;
}
...
function getSumOfNumbers(x1, x2, x3) {
  ...
  if (!x1 % 1 === 0 || !x2 % 1 === 0 || !x3 % 1 === 0) {
    return arr.reduce((sum, element) => sum + element, 0).toFixed(2);
  }
  return arr.reduce((sum, element) => sum + element, 0);
}
...
function getCountOfOddNumbers(number) {
  let count = 0;
  const newNumber = Math.sqrt(number * number);
